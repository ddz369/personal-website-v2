...
const Navbar = () => {
  ...
  return (
    <>
      <nav className="fixed right-5 top-1/2 transform -translate-y-1/2 z-50 hidden md:block">
              ...
              <Link key={link.name} href={link.path} className="group relative">
                <div
                  className={`relative flex items-center justify-center rounded-full p-2 transition-all duration-300 hover:bg-indigo-900/50 ${
                    isActive ? 'bg-indigo-800/50 text-indigo-200' : 'text-gray-400 hover:text-white'
                  }`}
                >
                  <Icon
                    size={22}
